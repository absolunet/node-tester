...
class Linters {
	...
	bash(paths = patterns.bash, { cwd = process.cwd() } = {}) {
		...
		globAll.sync(paths, { nodir:true, cwd:cwd }).forEach((file) => {
			test(`Bash syntax check on ${file}`, (t) => {
				return new Promise((resolve) => {
					exec(`bash -n ${fs.realpathSync(`${cwd}/${file}`)}`, {}, (err/* , stdout, stderr */) => {
						resolve(err);
					});
				}).then((err) => {
					if (err) {
	...
	editorconfig(paths = patterns.editorconfig, { cwd = process.cwd() } = {}) {
		...
		globAll.sync(paths, { nodir:true, cwd:cwd, dot:true }).forEach((file) => {
			test.cb(`EditorConfig check on ${file}`, (t) => {
				vfs.src(`${cwd}/${file}`)
					.pipe(eclint.check())
					.pipe(reporter({
						blame: false,
