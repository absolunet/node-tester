			...
			if (success) {
				...
				if (!ignore.includes(TEST)) {
					if (Object.keys(packageConfig).includes('scripts')) {
						...
						t.true(packageConfig.scripts.test.startsWith('ava test') || packageConfig.scripts.test.startsWith('npm run test:'), `Scripts > Test must start 'ava test' or 'npm run test:'`);

						if (scope === 'multi') {
							t.true(Object.keys(packageConfig.scripts).includes('postinstall'), `Scripts must contain postinstall`);
							t.is(packageConfig.scripts.postinstall, 'rm -rf packages/*/package-lock.json; lerna clean --yes; lerna bootstrap --no-ci; node manager.js --task=install', `Scripts > postinstall must be identical to manager definition`);
						} else {
							t.true(Object.keys(packageConfig.scripts).includes('installer'), `Scripts must contain installer`);
							t.is(packageConfig.scripts.installer, 'node manager.js --task=install', `Scripts > installer must be identical to manager definition`);
						}
