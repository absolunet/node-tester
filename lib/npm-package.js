...
const matrix = (filename) => {
	const ROOT = `${__dirname}/..`;
	const overwritePath = `${ROOT}/matrix/${filename.replace(/^\.(.*)/, `$1`)}`;

	if (fs.existsSync(overwritePath)) {
		return fs.realpathSync(overwritePath);
	}

	return fs.realpathSync(`${ROOT}/${filename}`);
};
...
class NpmPackage {
	...
	validateEslintConfig({ cwd = process.cwd() } = {}) {
		...
		ava.test('ESLintConfig: Check that package.json is valid', (t) => {
			...
			t.true(Array.isArray(PACKAGE.keywords), 'Keywords must be defined');
