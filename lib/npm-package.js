...
const getOptions = ({ cwd, group, scope, ignore = [] }) => {
	return {
		...
		group,
		path:          cwd,
		readablePath:  cwd.startsWith(process.cwd()) ? cwd.slice(process.cwd().length + 1) : cwd,
...
const configJsonTests = (config, t) => {
	...
	t.truthy(config.main || config.browser, `Main or browser must be defined`);
...
const testPackageJson = ({ path, readablePath, group, scope, packageConfig, ignore = [] }) => {
			...
			if (success) {
				...
				if (!ignore.includes(TEST)) {
					if (Object.keys(packageConfig).includes('scripts')) {
						...
						t.true(packageConfig.scripts.test.startsWith('ava test/index.js') || packageConfig.scripts.test.startsWith('npm run test:'), `Scripts > Test must start 'ava test/index.js' or 'npm run test:'`);

						if (scope === 'multi') {
							t.true(Object.keys(packageConfig.scripts).includes('postinstall'), `Scripts must contain postinstall`);
							t.is(packageConfig.scripts.postinstall, 'node manager.js --task=postinstall', `Scripts > postinstall must be identical to manager definition`);
						} else {
							t.true(Object.keys(packageConfig.scripts).includes('installer'), `Scripts must contain installer`);
							t.is(packageConfig.scripts.installer, 'node manager.js --task=installer', `Scripts > installer must be identical to manager definition`);
						}

						t.true(Object.keys(packageConfig.scripts).includes('outdated'), `Scripts must contain outdated`);
						t.is(packageConfig.scripts.outdated, 'node manager.js --task=outdated', `Scripts > outdated must be identical to manager definition`);

						t.true(Object.keys(packageConfig.scripts).includes('build'), `Scripts must contain build`);
						t.is(packageConfig.scripts.build, 'node manager.js --task=build', `Scripts > build must be identical to manager definition`);

						t.true(Object.keys(packageConfig.scripts).includes('deploy'), `Scripts must contain deploy`);
						t.is(packageConfig.scripts.deploy, 'node manager.js --task=deploy', `Scripts > deploy must be identical to manager definition`);
...
class NpmPackage {
	...
	validate({ cwd = process.cwd(), group, scope, js, json, yaml, bash, editorconfig, scss, ignore = [] } = {}) {
		...
		if (!ignore.includes(ESLINTRC)) {
			...
			testContainsMatrix('.eslintrc.yaml', options);
		}
	...
	validateSub({ cwd = process.cwd(), group, js, json, yaml, bash, editorconfig, scss } = {}) {
		const options = getOptions({
			...
			group,
			scope:  'sub',
			ignore: [EDITORCONFIG, ESLINTRC, GITIGNORE, MANAGER, TRAVIS, TEST]
		});
	...
	validateLibrary({ cwd = process.cwd(), group } = {}) {
		const options = getOptions({
			...
			group,
			scope:  'library',
			ignore: [EDITORCONFIG, ESLINTRC, GITIGNORE, TRAVIS, TEST]
		});
