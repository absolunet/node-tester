...
const getOptions = ({ cwd, scope, ignore = [] }) => {
	return {
		cwd:          cwd,
		path:         cwd,
...
const testExists = (filename, { path, readablePath }) => {
	ava.test(`Check that '${readablePath}/${filename}' exists`, (t) => {
		t.true(fss.exists(`${path}/${filename}`), `Must exists`);
	});
};


const testMatrix = (filename, { path, readablePath, scope }) => {
	ava.test(`Check that '${readablePath}/${filename}' is identical to matrix`, (t) => {
		t.is(fss.readFile(`${path}/${filename}`, 'utf8'), fss.readFile(matrix(filename, scope), 'utf8'), `Must be identical to matrix`);
	});
};


const testContainsMatrix = (filename, { path, readablePath, scope }) => {
	ava.test(`Check that '${readablePath}/${filename}' contains at least matrix`, (t) => {
...
const testPackageJson = ({ path, readablePath, pkg, ignore = [] }) => {
	ava.test(`Check that '${readablePath}/package.json' is valid and contains minimum information`, (t) => {
			...
			if (success) {
				...
				if (!ignore.includes(TEST)) {
					if (Object.keys(pkg).includes('scripts')) {
						...
						t.truthy(pkg.scripts.test, `Scripts > Test must not be empty`);
					} else {
...
const testBowerJson = (filename, { path, readablePath, pkg }) => {
	const config = fss.readJson(`${path}/${filename}`);

	ava.test(`Check that '${readablePath}/${filename}' is valid and contains minimum information`, (t) => {
	...
	ava.test(`Check that '${readablePath}/${filename}' is in sync with 'package.json'`, (t) => {
...
class NpmPackage {
	...
	//-- Validate
	validate({ cwd = process.cwd(), scope, js, json, yaml, bash, editorconfig, ignore = [] } = {}) {
		const selfTest = fss.realpath(cwd) === fss.realpath(`${__dirname}/..`);

		const options = getOptions({ cwd, scope, ignore });


		// Linters
		linters.js(js, { cwd });
		linters.json(json, { cwd });
		linters.yaml(yaml, { cwd });
		linters.bash(bash, { cwd });
		linters.editorconfig(editorconfig, { cwd });
		...
		// --verbose warning
		if (!ignore.includes(TEST)) {
			ava.test.after(`Check for verbose in '${options.readablePath}/package.json'`, () => {
	...
	validateMulti({ cwd = process.cwd() } = {}) {
		...
		options.js           = patterns.js.concat([`!${packagesPath}/**`]);
		options.json         = patterns.json.concat([`!${packagesPath}/**`]);
		options.yaml         = patterns.yaml.concat([`!${packagesPath}/**`]);
		options.bash         = patterns.bash.concat([`!${packagesPath}/**`]);
		options.editorconfig = patterns.editorconfig.concat([`!${packagesPath}/**`]);
		...
		ava.test(`Multi: Check that '${options.readablePath}/package.json' is valid`, (t) => {
		...
		ava.test(`Multi: Check that '${options.readablePath}/lerna.json' is valid`, (t) => {
	...
	validateEslintConfig({ cwd = process.cwd() } = {}) {
		...
		// Validate config
		ava.test(`ESLintConfig: Check that config is valid for '${options.readablePath}'`, (t) => {
		...
		ava.test(`ESLintConfig: Check that '${options.readablePath}/package.json' is valid`, (t) => {
	...
	validateStylelintConfig({ cwd = process.cwd() } = {}) {
		const options = getOptions({
			cwd:   cwd,
		...
		// Validate config
		ava.test(`StylelintConfig: Check that config is valid for '${options.readablePath}'`, (t) => {
			const data = require(cwd);  // eslint-disable-line global-require
			t.deepEqual(typeof data, 'object', 'YAML is not parsable');
		...
		// Extra validation on package.json
		ava.test(`StylelintConfig: Check that '${options.readablePath}/package.json' is valid`, (t) => {
	...
	validateLibrary({ cwd = process.cwd() } = {}) {
		...
		// Extra validation on package.json
		ava.test(`Library: Check that '${options.readablePath}/package.json' is valid`, (t) => {
