...
const testExists = (filename, { path, readablePath }) => {
	test(`Check that '${readablePath}/${filename}' exists`, (t) => {
...
const testMatrix = (filename, { path, readablePath, scope }) => {
	test(`Check that '${readablePath}/${filename}' is identical to matrix`, (t) => {
...
const testContainsMatrix = (filename, { path, readablePath, scope }) => {
	test(`Check that '${readablePath}/${filename}' contains at least matrix`, (t) => {
...
const testPackageJson = ({ path, readablePath, pkg, ignore = [] }) => {
	test(`Check that '${readablePath}/package.json' is valid and contains minimum information`, (t) => {
...
const testBowerJson = (filename, { path, readablePath, pkg }) => {
	...
	test(`Check that '${readablePath}/${filename}' is valid and contains minimum information`, (t) => {
	...
	test(`Check that '${readablePath}/${filename}' is in sync with 'package.json'`, (t) => {
...
class NpmPackage {
	...
	validateMulti({ cwd = process.cwd(), js = patterns.js, json = patterns.json, yaml = patterns.yaml, bash = patterns.bash, editorconfig = patterns.editorconfig, scss } = {}) {
		...
		test(`Multi: Check that '${options.readablePath}/package.json' is valid`, (t) => {
		...
		test(`Multi: Check that '${options.readablePath}/lerna.json' is valid`, (t) => {
	...
	validateEslintConfig({ cwd = process.cwd() } = {}) {
		...
		// Validate config
		test(`ESLintConfig: Check that config is valid for '${options.readablePath}'`, (t) => {
		...
		test(`ESLintConfig: Check that '${options.readablePath}/package.json' is valid`, (t) => {
	...
	validateStylelintConfig({ cwd = process.cwd() } = {}) {
		...
		// Validate config
		test(`StylelintConfig: Check that config is valid for '${options.readablePath}'`, (t) => {
			const data = require(cwd);  // eslint-disable-line global-require
			t.deepEqual(typeof data, 'object', 'YAML is not parsable');
		...
		// Extra validation on package.json
		test(`StylelintConfig: Check that '${options.readablePath}/package.json' is valid`, (t) => {
	...
	validateLibrary({ cwd = process.cwd() } = {}) {
		...
		// Extra validation on package.json
		test(`Library: Check that '${options.readablePath}/package.json' is valid`, (t) => {
