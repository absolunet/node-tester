...
class Linters {
	...
	js(paths = patterns.js, { cwd = process.cwd(), group, configFile, configPreset } = {}) {
		const cli = new CLIEngine({ configFile: configFile || configPreset || undefined, reportUnusedDisableDirectives: true });

		globAll.sync(paths, { nodir: true, cwd }).forEach((file) => {
	...
	json(paths = patterns.json, { cwd = process.cwd(), group } = {}) {
		...
		globAll.sync(paths, { nodir: true, dot: true, cwd }).forEach((file) => {
	...
	editorconfig(paths = patterns.editorconfig, { cwd = process.cwd(), group } = {}) {
		...
		globAll.sync(paths, { nodir: true, cwd, dot: true }).forEach((file) => {
