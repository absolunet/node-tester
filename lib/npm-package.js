	...
	ava.test.cb(`Check that '${filename}' is valid and contains minimum information`, (t) => {
		...
		readJson(`${CWD}/${filename}`, readJson.log, true, (error, config) => {
			...
			if (!error) {
				...
				t.regex(PACKAGE.name, /^@absolunet\/([a-z][a-z0-9]*)(-[a-z0-9]+)*$/u, `Name must be valid`);  // eslint-disable-line unicorn/no-unsafe-regex
...
class NpmPackage {
	...
	validate({ cwd = process.cwd(), scope, js, json, yaml, bash, editorconfig } = {}) {
		...
		ava.test.after('Check for verbose', () => {
			if ((/--verbose/u).test(PACKAGE.scripts.test)) {
	...
	validateEslintConfig({ cwd = process.cwd() } = {}) {
		...
		ava.test('ESLintConfig: Check that package.json is valid', (t) => {
			...
			t.regex(PACKAGE.name, /^@absolunet\/eslint-config-([a-z][a-z0-9]*)(-[a-z0-9]+)*$/u, `Name must be valid`);  // eslint-disable-line unicorn/no-unsafe-regex
	...
	validateStylelintConfig({ cwd = process.cwd() } = {}) {
		...
		// Validate config
		ava.test('StylelintConfig: Check that config is valid', (t) => {
			const data = require(CWD);  // eslint-disable-line global-require
			t.deepEqual(typeof data, 'object', 'YAML is not parsable');
		...
		// Extra validation on package.json
		ava.test('StylelintConfig: Check that package.json is valid', (t) => {

			t.regex(PACKAGE.name, /^@absolunet\/stylelint-config-([a-z][a-z0-9]*)(-[a-z0-9]+)*$/u, `Name must be valid`);  // eslint-disable-line unicorn/no-unsafe-regex
