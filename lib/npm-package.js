	...
	ava.test.cb(`Check that '${filename}' is valid and contains minimum information`, (t) => {
		...
		readJson(`${CWD}/${filename}`, readJson.log, true, (error, config) => {
			...
			if (!error) {
				...
				// Validate fields
				t.regex(PACKAGE.name, /^@absolunet\/([a-z][a-z0-9]*)(-[a-z0-9]+)*$/, `Name must be valid`);
				...
				if (Object.keys(PACKAGE).includes('repository')) {
					...
					t.regex(PACKAGE.repository.url, /git:\/\/github.com\/absolunet\/([a-z][a-z0-9]*)(-[a-z0-9]+)*\.git/, `Repository url is not valid`);
				} else {
				...
				if (Object.keys(PACKAGE).includes('bugs')) {
					...
					t.regex(PACKAGE.bugs.url, /https:\/\/github.com\/absolunet\/([a-z][a-z0-9]*)(-[a-z0-9]+)*\/issues/, `Bugs url is not valid`);
				} else {
